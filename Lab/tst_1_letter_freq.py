...
def testA():
    ...
    for word in WORD_OCCURRENCES:
        print('Total occurrences of', word + ':',
              'OK' if text_stats.totalOccurrences(word, words) == WORD_OCCURRENCES[word] \
                else 'GOT: ' + str(text_stats.totalOccurrences(word, words)) +
                     ', EXPECTED: ' + str(WORD_OCCURRENCES[word]))

    freqString = text_stats.letterFreq(words)
